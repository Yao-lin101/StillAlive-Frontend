...
const SlotComment: React.FC<{ slot: ReportCommentSlot; color: string }> = ({ slot, color }) => (
  <div style={{
    ...
    <div style={{ fontSize: '12px', fontWeight: 700, color, marginBottom: '6px', fontFamily: 'monospace' }}>{slot.range}</div>
...
export const ArisuTemplate: React.FC<TemplateProps> = ({ data, date }) => {
          ...
          <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            {(llm.schedule_slots?.length ?? 0) > 0 && (
              <div style={{ marginTop: '12px' }}>
                ...
                {llm.schedule_slots?.map((slot, i) => <SlotComment key={i} slot={slot} color={AQUA_THEME.sea} />)}
              </div>
            )}
          </div>
        );
              ...
              <>
                ...
                {(llm.activity_slots?.length ?? 0) > 0 && (
                  <div style={{ marginTop: '12px' }}>
                    ...
                    {llm.activity_slots?.map((slot, i) => <SlotComment key={i} slot={slot} color={AQUA_THEME.accent} />)}
                  </div>
                )}
              </>
            ) : (
          ...
          <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            {(llm.findings_slots?.length ?? 0) > 0 && (
              <div style={{ marginTop: '12px' }}>
                ...
                {llm.findings_slots?.map((slot, i) => <SlotComment key={i} slot={slot} color="#F59E0B" />)}
              </div>
            )}
          </div>
        );
            ...
            {displayItems.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                {displayItems.map((item, i) => (
                  <GlassCard key={i} style={{ padding: '20px', borderLeft: `4px solid ${AQUA_THEME.sea}` }}>
                    ...
                    <div style={{ fontSize: '14px', color: AQUA_THEME.textMain, lineHeight: 1.6 }}>{item.comment}</div>
                  </GlassCard>
                ))}
              </div>
            ) : (
  ...
  return (
    <div className="alice-template-container custom-scrollbar">
      <style>{`
        @font-face {
